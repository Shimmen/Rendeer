	...
	: TextureBase()
{
	...
	// Add the images to the texture cube
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, GL_RGB, left.GetWidth(), left.GetHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, left.GetData().data());
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, GL_RGB, right.GetWidth(), right.GetHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, right.GetData().data());
	//glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, GL_RGB, down.GetWidth(), down.GetHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, down.GetData().data());
	//glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, GL_RGB, up.GetWidth(), up.GetHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, up.GetData().data()); // FIXME: why is everything reversed? See shader as well.
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, GL_RGB, up.GetWidth(), up.GetHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, up.GetData().data());
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, GL_RGB, down.GetWidth(), down.GetHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, down.GetData().data());
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, GL_RGB, front.GetWidth(), front.GetHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, front.GetData().data());
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, GL_RGB, back.GetWidth(), back.GetHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, back.GetData().data());
